...
export default function Screen2_Wizard({ onBack, onComplete }: { onBack: () => void, onComplete: () => void }) {
 ...
 isActive ? "bg-blue-600 border-2 border-transparent text-white shadow-md" : "bg-linear-surface border-2 border-linear-border text-linear-text-muted font-medium"
 )}>
 {isActive && !isCurrent ? <CheckCircle2 className="w-4 h-4 text-white" /> : s}
 </div>
 <span className={cn("text-[10px] uppercase tracking-widest mt-2 whitespace-nowrap hidden md:block font-medium", isActive ? "text-blue-700 font-bold" : "text-linear-text-muted")}>{t(label)}</span>
 </div>
 );
 })}
 </div>
 ...
 {/* Navigation Footer */}
 <div className="sticky bottom-0 bg-linear-surface/90 backdrop-blur-md border-t border-linear-border p-6 flex justify-between items-center z-50">
 <div className="max-w-4xl mx-auto w-full flex justify-between items-center">
 <button onClick={handlePrev} className="px-6 py-3 rounded-xl border border-linear-border bg-linear-surface text-foreground font-medium hover:bg-linear-surface/80 transition-colors flex items-center shadow-sm">
